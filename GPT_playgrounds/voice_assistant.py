...
Assistant is aware that human input is being transcribed from audio and as such there may be some errors in the 
transcription. It will attempt to account for some words being swapped with similar-sounding words or phrases. 
Assistant will also keep responses concise, because human attention spans are more limited over the audio channel 
since it takes time to listen to a response.
    ...
    with sr.Microphone() as source:
        ...
        while (1):
            ...
            try:
                ...
                recognized_audio = r.recognize_whisper(audio, model='medium.en', show_dict=True,)
                if recognized_audio:
                    text = recognized_audio['text']
                else:
                    text = "Sorry, I couldn't recognize that."
